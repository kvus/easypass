...
> 🔒 **Rate Limiting:**
> - `POST /api/login` — **10 requests/phút** per IP.
> - `POST /api/register` — **5 requests/giờ** per IP.
>
> Vượt quá sẽ nhận `429 Too Many Requests`.
...
| `429` | `{"message": "Quá nhiều yêu cầu đăng ký, vui lòng thử lại sau 1 giờ"}` | Rate limit (5 req/giờ) |
